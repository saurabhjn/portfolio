...
def save_transactions_to_json(filepath: str, transactions_data: Dict[str, List[Transaction]]):
        ...
        for inv_name, transactions_list_data in data.items():
            transactions_data[inv_name] = [
                Transaction(
                    ...
                    sell_date=datetime.date.fromisoformat(item['sell_date']) if item.get('sell_date') else None,
                    sell_quantity=Decimal(item['sell_quantity']) if item.get('sell_quantity') is not None else None,
                    sell_rate=Decimal(item['sell_rate']) if item.get('sell_rate') is not None else None,
                    gain_from_sale=Decimal(item['gain_from_sale']) if item.get('gain_from_sale') is not None else None,
